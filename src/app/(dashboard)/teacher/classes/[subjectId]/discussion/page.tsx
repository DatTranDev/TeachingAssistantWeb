...
export default function TeacherDiscussionPage() {
  ...
  const resolveMutation = useMutation({
    mutationFn: ({ id, resolved }: { id: string; resolved: boolean }) =>
      questionsApi.update(id, { isResolved: resolved }),
    onMutate: async ({ id, resolved }) => {
      ...
      queryClient.setQueryData(
        queryKeys.questions.bySubject(subjectId),
        (old: Question[] | undefined) =>
          old?.map((q) => (q._id === id ? { ...q, isResolved: resolved } : q)) ?? []
      );
      return { snapshot };
    },
    ...
    onSuccess: (_data, { id, resolved }) => {
      if (resolved && socket) {
        socket.emit('sendResolve', { subjectID: subjectId, messageID: id });
      }
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    ...
                    <Popover>
                      ...
                      <PopoverContent align="start" sideOffset={8} className="w-72 p-0 shadow-xl bg-white dark:bg-slate-900 border border-neutral-200 dark:border-slate-700">
                        {author ? (
                              ...
                              <div className="min-w-0 flex-1">
                                ...
                                {author.userCode && (
                                  <p className="text-xs text-muted-foreground font-medium mt-0.5">
                                    ...
                                    {author.role === 'student' ? 'Sinh viên' : 'Giảng viên'}
                                  </p>
                                )}
                              </div>
                            </div>
                ...
                <div className="flex items-center justify-between gap-2 pt-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <Button
                      ...
                      size="sm"
                      className={cn(
                        'h-7 text-xs gap-1 hover:bg-neutral-100 dark:hover:bg-slate-700',
                        q.isResolved &&
                          'text-green-700 dark:text-green-400 border-green-200 dark:border-green-800'
                      )}
                      onClick={() => resolveMutation.mutate({ id: q._id, resolved: !q.isResolved })}
                      disabled={resolveMutation.isPending}
                    >
                      <CheckCircle2 className="h-3.5 w-3.5" />
                      {q.isResolved ? t('discussion.unresolvedToast') : t('discussion.resolved')}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
